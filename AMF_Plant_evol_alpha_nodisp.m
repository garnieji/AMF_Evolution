...
%     end
    ...
    fp = @(alpha,P,M)  P.*(rp+ (Q*sum(alpha.*M*dalpha)./(d+P) -beta*sum(M*dalpha))-mup*P);
...
for im = 1:length(Id)
    ...
    plot(BETA(id),Abar(id),'xk', 'MarkerSize', 10,'LineWidth',2)%,'MarkerEdgeColor',redgradient(im,:)); 
end
...
for im = 1:length(Id)
    ...
    plot(Abar(Id(im)),MM_d_approx(Id(im),ia),'xk', 'MarkerSize', 10,'LineWidth',2)%,'MarkerEdgeColor',redgradient(im,:)); 
end
hl = legend(p,dp,'FontSize',16)
...
% saveas(gca,'fig_distrib_alpha_rp0_bis.eps','epsc')
